...
def product(request, pid):
	products = Product.objects.select_related('author').get(id=pid)
...
def cart_update(request, pid):
	...
	if cart_model is None:
		...
		cart_model.save()
	return JsonResponse({
		'message': _('The product add to your cart.'),
		'item_count': len(cart_model.items)
...
def cart_remove(request, pid):
	...
	return JsonResponse({
		'message': 'The product has removed from your cart',
